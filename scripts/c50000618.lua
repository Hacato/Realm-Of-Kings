...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Damage when Wyrm destroys opponent's monster
...
function s.spfilter(c,e,tp)
	return c:IsLevelBelow(3) and c:IsRace(RACE_WYRM) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
...
function s.damcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tc:IsRelateToBattle() and tc:IsControler(tp) and tc:IsRace(RACE_WYRM)
