...
--(1) Search
function c99890100.thcostfilter(c)
...
function c99890100.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  if chk==0 then
    return not c:IsPublic()
      and Duel.IsExistingMatchingCard(c99890100.thcostfilter,tp,LOCATION_DECK,0,1,nil)
  end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
  local g=Duel.SelectMatchingCard(tp,c99890100.thcostfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c99890100.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then
    return Duel.IsExistingMatchingCard(c99890100.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil)
  end
...
function c99890100.spreg(e,tp,eg,ep,ev,re,r,rp)
  ...
  if Duel.GetTurnPlayer()==tp and Duel.GetCurrentPhase()==PHASE_STANDBY then
    ...
    c:RegisterFlagEffect(99890101,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,0,2)
  else
    e:SetLabel(0)
    c:RegisterFlagEffect(99890101,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,0,1)
  end
end
...
function c99890100.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then
    return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1
      and Duel.IsExistingMatchingCard(c99890100.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp)
  end
  Duel.Hint(HINT_OPSELECTED,1-tp,e:GetDescription())
...
function c99890100.cbtop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if c:IsRelateToEffect(e) and at and not at:IsImmuneToEffect(e) and c:IsFaceup() then
