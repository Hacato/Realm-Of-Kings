...
function s.initial_effect(c)
  ...
  -- Pendulum Effects
  -------------------------------
  --(1) Destroy self → add 1 "YuYuYu" Ritual Spell
  ...
  --(2) Place another Pendulum from Deck into PZone
  ...
  --(2) Gain ATK for each "YuYuYu" Ritual Monster
...
-- Place Pendulum from Deck into PZone
...
function s.ppztg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then
    return (Duel.CheckLocation(tp,LOCATION_PZONE,0) or Duel.CheckLocation(tp,LOCATION_PZONE,1))
       and Duel.IsExistingMatchingCard(s.ppzfilter,tp,LOCATION_DECK,0,1,nil)
  end
end
...
-- Gain ATK for each "YuYuYu" Ritual Monster
function s.atkfilter(c)
  return c:IsFaceup() and c:IsSetCard(0x991) and c:IsType(TYPE_RITUAL+TYPE_MONSTER)
end
