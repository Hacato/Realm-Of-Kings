...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- Fixed line: Added a third parameter (0) to IsCanBeSpecialSummoned in the filter
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		--Half damage when attacking directly
		local e2=Effect.CreateEffect(c)
		e2:SetType(EFFECT_TYPE_SINGLE)
		e2:SetCode(EFFECT_CHANGE_BATTLE_DAMAGE)
		e2:SetCondition(function(e) return Duel.GetAttackTarget()==nil end)
		e2:SetValue(aux.ChangeBattleDamage(1-tp,HALF_DAMAGE))
		e2:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
		tc:RegisterEffect(e2)
		
		--Fallback in case the previous effect doesn't work
		local e3=Effect.CreateEffect(c)
		e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		e3:SetCode(EVENT_PRE_DAMAGE_CALCULATE)
		e3:SetCondition(function(e,tp,eg,ep,ev,re,r,rp) return Duel.GetAttacker()==tc and Duel.GetAttackTarget()==nil end)
		e3:SetOperation(function(e,tp,eg,ep,ev,re,r,rp) Duel.ChangeBattleDamage(1-tp,math.floor(Duel.GetBattleDamage(1-tp)/2)) end)
