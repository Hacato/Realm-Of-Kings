...
function s.filter(c,e,tp)
	return c:IsSetCard(0x711) and c:IsType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_DECK,0,1,c,e,tp,c:GetRace())
end
function s.filter2(c,e,tp,race)
	return c:IsSetCard(0x711) and c:IsType(TYPE_MONSTER) and not c:IsRace(race) and (c:IsAbleToHand() or c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEDOWN_DEFENSE))
end
