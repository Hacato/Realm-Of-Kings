...
function s.initial_effect(c)
  ...
  --(2) Return self to hand, place another Pendulum from Extra to scale
  ...
  -- Monster Effects
  -------------------------------
  --(1) Tribute self to add 1 "YuYuYu" Ritual Monster (except itself)
  ...
  --(2) Double damage when attacking
  ...
  --(3) Leave-field trigger: Add Ritual Spell from GY + Recover 500 LP
...
-- Return to hand and place another Pendulum
...
function s.rthtg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then
    return e:GetHandler():IsAbleToHand() and Duel.IsExistingMatchingCard(s.ppzfilter,tp,LOCATION_EXTRA,0,1,nil)
  end
...
-----------------------------------
-- Tribute to search Ritual Monster
...
function s.ddop(e,tp,eg,ep,ev,re,r,rp)
  ...
  e1:SetCondition(function(e,tp,eg,ep,ev,re,r,rp) return ep~=tp and e:GetHandler():GetBattleTarget()~=nil end)
  e1:SetOperation(function(e,tp,eg,ep,ev,re,r,rp) Duel.ChangeBattleDamage(ep,Duel.GetBattleDamage(ep)*2) end)
  e1:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
  c:RegisterEffect(e1)
end

-- Leave-field search Ritual Spell + Recover
