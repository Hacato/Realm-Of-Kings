...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SelectEffectYesNo(tp,e:GetHandler(),95) then
		...
		local g2=Duel.GetMatchingGroup(function(c) return c:IsFaceup() and c:IsSetCard(0x719) and c:IsType(TYPE_MONSTER) end,tp,LOCATION_MZONE,0,nil)
...
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)
		and c:IsSetCard(0x719) and c:IsType(TYPE_MONSTER)
