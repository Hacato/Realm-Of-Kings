...
--Opponent cannot chain to Zombie monster effect activations
function s.aclimit(e,re,tp)
	-- Check if there's a current chain
	if Duel.GetCurrentChain()==0 then return false end
	-- Get the effect that was just activated (the one opponent would be chaining to)
	local chain_count=Duel.GetCurrentChain()
	local te,teg,tep=Duel.GetChainInfo(chain_count-1,CHAININFO_TRIGGERING_EFFECT)
	-- If the previous chain link was a Zombie monster effect, prevent opponent from chaining
	if te and te:IsActiveType(TYPE_MONSTER) and te:GetHandler():IsRace(RACE_ZOMBIE) then
		return true
	end
	return false
end
