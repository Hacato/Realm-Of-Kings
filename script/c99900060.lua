...
function c99900060.initial_effect(c)
  ...
  e3:SetDescription(aux.Stringid(99900060,0))
  e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
  e3:SetType(EFFECT_TYPE_IGNITION)
  e3:SetRange(LOCATION_FZONE)
  e3:SetCondition(c99900060.spcon)
  e3:SetTarget(c99900060.sptg)
  e3:SetOperation(c99900060.spop)
  c:RegisterEffect(e3)
...
function c99900060.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  if g1:GetCount()>0 and g2:GetCount()>0 then
    for tc in aux.Next(g1) do
      g2:DeleteGroup(Card.IsCode,tc:GetCode())
    end
  end
...
function c99900060.spop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD)~=0 and g2:GetCount()>0 then
    ...
    if tc then
      if Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP)~=0 then
        ...
        tc:RegisterFlagEffect(99900060,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(99900060,3))
...
function c99900060.thcost2(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  Duel.DiscardHand(tp,c99900060.thcostfilter2,1,1,REASON_COST+REASON_DISCARD)
end
